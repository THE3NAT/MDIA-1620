...
let customers = [{name: "sally", age: 2}, ["sally", 2]]


// customers.push(human1, human2, human3, human4, human5, human6, human7, human8)

let settings = {breakableItems: true,
    ageLimit: 11;
};


function ageCheck(ageLimit){
    for(let singleCustomer of customers){
        if (breakableItems === false){
            return console.log ("Everyone come inside and spend money!")
        }

        if (singleCustomer.age <= ageLimit && breakableItems === true){
            return console.log (`Sorry, ${singleCustomer.name}but your're only ${singleCustomer.age} years old. You need to be at least 10 years old to enter. Please wait in line`)
        } 
